...
#if defined(NDEBUG)
# error "Bolivarcoin Core cannot be compiled without assertions."
#endif
...
/*
NOTE:   unlike bolivarcoin we are using PREVIOUS block height here,
...
{
    double dDiff;
    CAmount nSubsidyBase = 25;

    // LogPrintf("height %u diff %4.2f reward %d\n", nPrevHeight, dDiff, nSubsidyBase);
    CAmount nSubsidy = nSubsidyBase * COIN;

    if (nPrevHeight < 500000) {
        nSubsidy = 25 * COIN;
...
void ThreadScriptCheck() {
    RenameThread("bolivarcoin-scriptch");
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    // BOLIVARCOIN : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(BOLIVARCOIN): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(BOLIVARCOIN): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // BOLIVARCOIN : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(BOLIVARCOIN): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(BOLIVARCOIN): spork is off, skipping transaction locking checks\n");
    }
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    if(Params().NetworkIDString() == CBaseChainParams::MAIN && nHeight <= 1){
        ...
        if (nHeight > 520000 && block.nBits != GetNextWorkRequired(pindexPrev, &block, consensusParams))
