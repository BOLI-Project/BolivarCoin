...
*
*   Why is this needed?
*   - In Bolivarcoin some blocks are superblocks, which output much higher amounts of coins
...
bool IsBlockPayeeValid(const CTransaction& txNew, int nBlockHeight, CAmount blockReward)
{
    ...
    if(sporkManager.IsSporkActive(SPORK_8_MASTERNODE_PAYMENT_ENFORCEMENT) && !mnpayments.IsTransactionValid(txNew, nBlockHeight)) {
		LogPrintf("IsBlockPayeeValid -- ERROR: Invalid bolivarcoin masternode payment detected at height %d: %s", nBlockHeight, txNew.ToString());
        return false;
    } else if (mnpayments.IsTransactionValid(txNew, nBlockHeight)){
		LogPrintf("IsBlockPayeeValid -- Valid bolivarcoin masternode payment detected at height %d: %s", nBlockHeight, txNew.ToString());
	}
...
{
    if(fLiteMode) return; // disable all Bolivarcoin specific functionality
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f BOLIVARCOIN\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
