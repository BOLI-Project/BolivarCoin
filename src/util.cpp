...
const char * const BITCOIN_CONF_FILENAME = "Bolivarcoin.conf";
const char * const BITCOIN_PID_FILENAME = "Bolivarcoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "bolivarcoin" is a composite category enabling all Bolivarcoin-related debug output
            if(ptrCategory->count(string("bolivarcoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Bolivarcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Bolivarcoin
    // Mac: ~/Library/Application Support/Bolivarcoin
    // Unix: ~/.Bolivarcoin
#ifdef WIN32
    // Windows
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Bolivarcoin";
#else
    // Unix
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty Bolivarcoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override Bolivarcoin.conf
