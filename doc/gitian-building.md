...
*Setup instructions for a Gitian build of Bolivarcoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Bolivarcoin
...
- [Building Bolivarcoin Core](#building-bolivarcoin-core)
...
Clone the git repositories for Bolivarcoin Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/BolivarcoinCore/Bolivarcoin
```
...
in the Bolivarcoin Core repository under 'Fetch and build inputs' to install sources which require
...
To build Bolivarcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Bolivarcoin Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bolivarcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/BolivarcoinCore/Bolivarcoin
...
For example:
```bash
URL=https://github.com/crowning-/bolivarcoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit bolivarcoin=${COMMIT} --url bolivarcoin=${URL} ../bolivarcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bolivarcoin=${COMMIT} --url bolivarcoin=${URL} ../bolivarcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bolivarcoin=${COMMIT} --url bolivarcoin=${URL} ../bolivarcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the bolivarcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../bolivarcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/bolivarcoin.git
SIGPATH=/some/root/path/bolivarcoin-detached-sigs.git

./bin/gbuild --url bolivarcoin=${BTCPATH},signature=${SIGPATH} ../bolivarcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bolivarcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bolivarcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/bolivarcoin-osx-build.assert
```
...
[bolivarcoin/gitian.sigs](https://github.com/bolivarcoinpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
