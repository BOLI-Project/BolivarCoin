...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Bolivarcoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Bolivarcoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Bolivarcoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Bolivarcoin 0.7 datadir."
    ;;
  3)
    echo "Detected Bolivarcoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Bolivarcoin 0.8 datadir."
    ;;
esac
